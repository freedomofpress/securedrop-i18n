...
function enhance_ui() {
  ...
  $('div#filter-container').html('<input id="filter" type="text" placeholder="' + get_string("filter_by_codename_placeholder_string") + '" autofocus >');

  // Add the "select {all,none}" buttons
  $('div#select-container').html('<span id="select_all" class="select"><i class="fa fa-check-square-o"></i> ' + get_string("select_all_string") + '</span> <span id="select_unread" class="select"><i class="fa fa-check-square-o"></i> ' + get_string("select_unread_string") + '</span> <span id="select_none" class="select"><i class="fa fa-square-o"></i> ' + get_string("select_none_string") + '</span>');

  $('div#index-select-container').replaceWith('<span id="select_all" class="select"><i class="fa fa-check-square-o"></i> ' + get_string("select_all_string") + '</span> <span id="select_none" class="select"><i class="fa fa-square-o"></i> ' + get_string("select_none_string") + '</span>');
...
$(function () {
  ...
  $("#delete_collection").submit(function () {
    return confirm(get_string("collection_delete_confirm_string"));
  });
  ...
  $("#delete_collections").click(function () {
    ...
    if (checked.length > 0) {
      return confirm(get_string("collection_multi_delete_confirm_string").supplant({ size: checked.length }));
    }
  ...
  $("#delete_selected").click(function () {
      ...
      if (checked.length > 0) {
          return confirm(get_string('submission_multi_delete_confirm_string').supplant({ size: checked.length }));
      }
  ...
  $('button.delete-user').click(function(event) {
      ...
      return confirm(get_string("delete_user_confirm_string").supplant({ username: username }));
  });

  // Confirm before resetting two-factor authentication on edit user page
  $('form#reset-two-factor').submit(function(event) {
      return confirm(get_string("reset_user_mfa_confirm_string").supplant({ username: username }));
  });
});
